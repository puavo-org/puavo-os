...
class WeatherSection extends St.Button {
    ...
    _updateForecasts() {
        const infos = this._client.getInfos(this._options.maxForecasts, this._options.intervalHour);
        // RTL support
...
class WeatherWidget extends St.BoxLayout {
    ...
    _updateStyleClass() {
        ...
        if (options.removeShadow)
            style += " QSTWEAKS-weather-remove-shadow";
        if (options.compact)
            style += " QSTWEAKS-weather-compact";
