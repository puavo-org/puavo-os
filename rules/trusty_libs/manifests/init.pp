class trusty_libs {
  ...
  define deb_unpack ($url, $sha384sum, $dest_subdir) {
    $srcpath    = $title
    $destdir    = "${::trusty_libs::basedir}/${dest_subdir}"
    $targetpath = inline_template("${destdir}/<%= File.basename(scope.lookupvar('srcpath')) %>")

    exec {
      "/usr/lib/puavo-pkg/unpack-a-file-from-deb ${url} ${sha384sum} ${destdir} ${srcpath}":
        creates => $targetpath,
