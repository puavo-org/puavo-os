...
def init_programs(programs, puavopkg_states, icon_size):
    for pid, program in programs.items():
        ...
        if program["puavopkg"]["id"] not in puavopkg_states:
            ...
            # uninstalled.
            logging.info(
                'puavo-pkg program "%s" is already part of the desktop image', pid
            )
...
def __program_uninstalled(program, icon_cache):
    ...
    if program.icon:
        ...
        if old_icon:
            logging.info('__program_uninstalled(): unloading old icon "%s"', old_icon)
    ...
    logging.info('__program_uninstalled(): restoring installer icon "%s"', icon_name)
    ...
    if not usable:
        logging.error(
            "__program_uninstalled(): unable to load the installer icon "
            '"%s" for removed program "%s"',
            icon_name,
            program.menudata_id,
        )
...
def __program_installed(program, language, desktop_dirs, icon_locator, icon_cache):
    ...
    if desktop_file is None:
        logging.error(
            '__program_installed(): .desktop file for program "%s" not found',
            program.menudata_id,
        )
        return False
    ...
    try:
        ...
        logging.error(
            '__program_installed(): Could not load file "%s" for program "%s": %s',
            desktop_file,
            program.menudata_id,
            str(exception),
        )
    ...
    if "Desktop Entry" not in desktop_data:
        logging.error(
            '__program_installed(): Rejecting desktop file "%s" for "%s": No "[Desktop Entry]" '
            "section in the file",
            desktop_file,
            program.menudata_id,
        )
...
def reload_program(
    ...
    if not isinstance(program, menudata.PuavoPkgProgram):
        logging.error(
            "Some idiot called puavopkg.reload_program with a non-puavopkg program object!"
        )
        return False
    ...
    if pkg_id not in puavopkg_states:
        logging.error(
            'reload_program(): program "%s" is part of the desktop image, '
            "doing nothing",
            pkg_id,
        )
        return False
    ...
    if (
        old_state == menudata.PuavoPkgState.INSTALLED
        and new_state == menudata.PuavoPkgState.NOT_INSTALLED
    ):
        # This program has been uninstalled
        logging.info(
            'reload_program(): puavo-pkg program "%s" has been uninstalled', pkg_id
        )
        ...
        logging.info(
            'reload_program(): puavo-pkg program "%s" has been installed', pkg_id
        )

        success = __program_installed(
            program, language, desktop_dirs, icon_locator, icon_cache
        )
