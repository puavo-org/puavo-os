    ...
    def run(self):
        ...
        for attempt in range(0, self.MAX_ATTEMPTS):
            try:
                ...
                logging.info(
                    'Downloading user avatar from "%s", attempt %d/%d...',
                    uri,
                    attempt + 1,
                    self.MAX_ATTEMPTS,
                )
                ...
                proc = subprocess.Popen(
                    ["puavo-rest-request", uri, "--user-krb"],
                ...
                if proc.returncode != 0:
                    raise RuntimeError(
                        "'puavo-rest-request' failed with code {0}".format(
                            proc.returncode
                        )
                    )
                ...
                logging.info(
                    "Downloaded %d bytes of avatar image data in %s ms",
                    len(image),
                    "{0:.1f}".format((end_time - start_time) * 1000.0),
                )
                ...
                try:
                    ...
                    logging.warning(
                        "Failed to save the downloaded avatar " "image: %s",
                        str(exception),
                    )
                ...
                logging.error("Could not download the user avatar: %s", str(exception))

            # Retry, if possible
            if attempt < self.MAX_ATTEMPTS - 1:
                logging.info(
                    "Retrying avatar download in %d seconds...", self.RETRY_WAIT
                )
                time.sleep(self.RETRY_WAIT)

        logging.error(
            "Giving up on trying to download the user avatar, " "tried %d times",
            self.MAX_ATTEMPTS,
        )
        logging.info("The avatar update thread is exiting")
