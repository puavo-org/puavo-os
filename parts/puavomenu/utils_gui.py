...
def rounded_rectangle(ctx, x, y, width, height, radius=20):
    ...
    ctx.arc(x + width - radius, y + radius, radius, radians(-90.0), radians(0.0))
    ctx.arc(
        x + width - radius, y + height - radius, radius, radians(0.0), radians(90.0)
    )
    ctx.arc(x + radius, y + height - radius, radius, radians(90.0), radians(180.0))
    ctx.arc(x + radius, y + radius, radius, radians(180.0), radians(270.0))
...
def create_desktop_link(filename, program):
    ...
    if not is_web:
        ...
        subprocess.Popen(
            ["gio", "set", filename, "metadata::trusted", "true"],
...
def create_panel_link(program):
    ...
    if isinstance(program, menudata.UserProgram):
        ...
        if program.original_desktop_file in panel_faves:
            logging.info(
                'User program "%s" is already in org.gnome.shell.favorite-apps, '
                "doing nothing",
                program.menudata_id,
            )
            return
        ...
        logging.info(
            'Appending user program "%s" to org.gnome.shell.favorite-apps',
    ...
    if desktop_name in panel_faves:
        if program.original_desktop_file is not None:
            logging.info(
                'Desktop file "%s" is already in the panel, ' "doing nothing",
                desktop_name,
            )
            return
    ...
    if program.original_desktop_file is None:
        ...
        if os.path.exists(name):
            logging.info(
                'Local .desktop file for "%s" ("%s") already exists', program.name, name
            )
        else:
            logging.info(
                'Creating a local .desktop file for "%s", ' 'name="%s"',
                program.name,
                name,
            )
            ...
            with open(name, "w", encoding="utf-8") as out:
                ...
                if program.original_icon_name is not None:
                    logging.info(
                        'Setting the panel icon to "%s"', program.original_icon_name
                    )
                    out.write("Icon={0}\n".format(program.original_icon_name))
                else:
                    if isinstance(program, menudata.WebLink):
                        ...
                        out.write("Icon=text-html\n")
                        logging.info(
                            "Using a generic icon for a web link without an icon"
                        )
                    else:
                        out.write("Icon=/opt/puavomenu/res/missing_icon.png\n")
                        logging.info(
                            "Using a generic icon for a panel launcher without an icon"
                        )
    ...
    if desktop_name not in panel_faves:
        logging.info(
            'Appending program "%s" ("%s") to org.gnome.shell.favorite-apps',
            program.menudata_id,
            desktop_name,
        )
