...
rootfs-image: check-buildhost $(rootfs_dir) $(image_dir)
	...
	$(_sudo) .aux/verify-boot-components --rootfs '$(rootfs_dir)' --key-directory './config/boot_keys' --manifest './config/boot-manifest.json'
...
.PHONY: verify-boot-signatures
verify-boot-components: $(rootfs_dir)
	$(_sudo) .aux/verify-boot-components \
	    --rootfs '$(rootfs_dir)' \
...
prepare:
	...
	$(_sudo) env 'FACTER_localmirror=$(CURDIR)/debs/.archive' \
	    'FACTER_puavoimage_class=$(image_class)' \
...
apply-rules: /etc/puavo-image/class
	...
	$(_sudo) env 'FACTER_localmirror=$(CURDIR)/debs/.archive' \
	    'FACTER_puavoimage_class=$(image_class)' \
	    'FACTER_puavoruleset=$(image_class)' \
	    .aux/apply-rules

.PHONY: rdiffs
rdiffs: $(image_dir) $(mirror_dir)
	$(_sudo) rake -j 4 -mf .aux/make-rdiffs.rake image_dir="$(image_dir)" \
